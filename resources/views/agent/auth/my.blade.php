                    ...
                    <div class="card-body p-5">
                        <form id="user-info-form" class="my_about">
                            ...
                            <div class="tab-content mb-5">
                                <div class="tab-pane{{ $tab == 1 ? ' show active' : '' }}" role="tabpanel" id="baseNavDemo" aria-labelledby="baseNavDemoTab">
                                    ...
                                    @include('partials.form.select', [
                                        ...
                                        'value' => $user->specialized_field,
                                        'default' => true,
                                        'options' => collect(App\Models\Agent\ServiceCategory::getCate())->map(function($cat) {
                                            return [
                                                'value' => $cat->id,
                                                'text' => $cat->text,
                                            ];
                            ...
                            <div class="text-right my_infoo">
                                {{-- <div>
                                    <button type="button" data-toggle="modal" data-target="#goodbye_user">@lang('button.goodbyeuser')</button>
                                    <ol>
                                        <li>
                                            @if($drop)
                                                {{ $drop->created_at }} (요청)
...
<div id="welcome_popup">
	...
	<div>
		...
		<h1>
            @lang('messages.secession.injeng')
		</h1>
	</div>
</div>
@endif
...
<div class="modal-dialog modal-dialog-centered">
	<div class="modal-content">
		<div class="modal-body">
            <h5>@lang('messages.secession.yocheng')</h5>
            <p>@lang('messages.secession.yochengp')</p>
			<select class="form-select goodbye_select" name="reason" onchange="" id="drop_reason">
                <option value="">@lang('messages.secession.sayou')</option>
			...
			<ul>
				<li><button  type="button" data-dismiss="modal">@lang('button.cancel')</button></li>
				<li><button type="button" data-dismiss="modal" onclick="goodbye_user()">@lang('button.goodbyeuser')</button></li>
			</ul>
...
<button type="button" id="already" data-toggle="modal" data-target="#goodbye_user1" style="display:none">@lang('button.already_goodbyeuser')</button>
<!-- modal -->
...
<div class="modal-dialog modal-dialog-centered">
	<div class="modal-content">
		<div class="modal-body">
            <h5>@lang('messages.secession.already_yocheng')</h5>
            <p>@lang('messages.secession.already_yochengp')</p>

			<ul>
				<li><button  type="button"  data-dismiss="modal">@lang('button.confirm')</button></li>
			</ul>
        ...
        $.ajax({
            ...
            success: (response) => {
                if(response.code == 200) {
                    ...
                    location.href = "{{ route('agent.my') }}"+'?tab='+tab;
                }
                else {
