let datatable = $("#datatable").DataTable({
    ...
    serverSide: true,
    ajax: {
        url: '/service/service.data',
        data: (data) => {
            data.columns[3].search.value = 'process-'+$('#datatable_wrapper [name=process]').val()
            data.columns[5].search.value = $('#datatable_filter [name=daterange]').val()
            return data
        },
        dataSrc: (response) => {
    ...
    columns: [
        ...
        {data: 'default_description.name', name: 'default_description.name', orderable:!1},
        ...
        {data: 'visible', name: 'visible', searchable:!1},
        {data: 'in_app_payment', name: 'in_app_payment', searchable:!1},
    ...
    fnDrawCallback: () => {
        $('#datatable').find('tbody tr td:not(.select-checkbox)').click(function() {
            ...
            location.href = `/service/service/${$(this).parents('tr').attr('id')}`
        });
    },
    initComplete: () => {
