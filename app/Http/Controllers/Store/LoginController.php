...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if($this->guard()->attempt($this->credentials($request), $remember_me))
        {
            ...
            if(!$this->guard()->user()->dropped_at){
                ...
                if(empty($byapps)){
                    $redirect = config('app.pre_url').'://'.($this->guard()->user()->type == 2 ? config('app.domain.agent') : config('app.domain.store'));
                }
    ...
    {
        return $request->only('account', 'password', 'type');
    }
    ...
    {
        return view('store.find_account', ['account' => $request->account]);
    }
    ...
    {
        return view('store.find_pw', ['phone' => $request->phone]);
    }
}
