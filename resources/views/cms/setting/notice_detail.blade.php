        ...
        <table>
            <tbody>
                <tr>
                    ...
                    <td>
                        <select class="form-control d-inline-block" id="type" name="type" style="width: 40%;">
                            <option value="0" {{$result->type == '0' ? 'selected' : ''}}>전체공지</option>
                            <option value="2" {{$result->type == '2' ? 'selected' : ''}}>제휴사공지</option>
                            <option value="1" {{$result->type == '1' ? 'selected' : ''}}>고객사공지</option>
                        </select>
                        <div class="form-check form-check-inline ml-3">
                            <input class="form-check-input" id="popup" name="popup" type="checkbox" value="1" {{$result->popup ? 'checked' : ''}} />
                ...
                <tr>
                    ...
                    <td>
                        <div class="position-relative" style="width:20rem">
                            <img src="{{ $result->img ? Storage::url($result->img) : '/assets/images/empty2.png' }}" width="100%" class="pointer" />
                ...
                <tr>
                    ...
                    <td>
                        <textarea class="scrollTextBox" name="content" id="content" style="width:calc( 100% - 80px );">{{$result->content}}</textarea>
    ...
    <div class="mt-4">
        <div class="col-12 row p-0 m-0" >
            <div class="col-12 text-right" style="padding-right:0;">
                <button type="button" onclick="deleteNotice()" class="btn waves-effect waves-light" style="background:red; color:#fff">삭제</button>
                <button type="button" onclick="location.href='notice'" class="btn waves-effect waves-light" style="background:#ccc">취소</button>
    ...
    function deleteNotice(){
        ...
        $.ajax({
            ...
            success: (response) => {
                if(response.code == 200){
                    alert('삭제되었습니다');
                    location.replace('/setting/notice');
                }
            },
            error: (e) => {
                console.log(e.responseJSON)
